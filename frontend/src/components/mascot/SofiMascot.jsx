...
// Altura deseada dentro de la escena (en unidades de Three.js). El .glb ya
// viene con una escala "real" horneada por el exportador (normalmente ronda
// los ~1.9), así que en vez de multiplicar por una constante fija —lo que
// antes duplicaba el tamaño y sacaba a Sofi del encuadre de la cámara—
// medimos el bounding box real del modelo cargado y calculamos el factor de
// escala necesario para que siempre quepa completo, sin importar cómo venga
// exportado el archivo la próxima vez.
...
function SofiModel({ reducedMotion, mood = 'neutral', talking = false }) {
  ...
  useEffect(() => {
    ...
    scene.traverse((obj) => {
      ...
      if (obj.isMesh) {
        ...
        if (obj.material) {
          ...
          mats.forEach((mat) => {
            // Sin un mapa de entorno (environment map), los materiales PBR muy
            // metálicos se ven casi negros/apagados porque no tienen nada que
            // reflejar. Sofi es un personaje ilustrado, no una superficie de
            // metal pulido, así que limitamos qué tan metálico puede leerse
            // para que el color del diffuse (la pintura del ilustrador) se
            // vea tal cual, con o sin luces de entorno.
            if ('metalness' in mat) mat.metalness = Math.min(mat.metalness ?? 1, 0.2);
    ...
    // Medir el modelo tal como llegó (con la escala/traslación que el propio
    // .glb trae en su nodo raíz) y calcular el factor para que siempre
    // termine con la misma altura visible en la cámara, sin depender de cómo
    // se haya exportado.
    const box = new THREE.Box3().setFromObject(scene);
    const size = box.getSize(new THREE.Vector3());
    const height = size.y > 0.001 ? size.y : 1;
    setModelScale(DISPLAY_HEIGHT / height);

    if (import.meta.env.DEV && Object.keys(bones).length === 0) {
      console.warn(
        '[SofiMascot] El modelo no tiene ningún hueso reconocible (CC_Base_*). ' +
        'Probablemente el .glb se exportó sin el esqueleto/skinning (Armature), ' +
        'así que las animaciones de brazos, cadera y boca no van a moverse. ' +
        'Sofi se mostrará en la pose estática que trae el archivo.'
      );
    }
  }, [scene]);
  ...
  return (
    <group ref={groupRef}>
      <Center>
        <group ref={rigRef} scale={modelScale}>
  ...
  return (
    <div className="w-full h-full">
      ...
      >
        ...
        <hemisphereLight args={['#fff6e8', '#7a5a3a', 0.6]} />
