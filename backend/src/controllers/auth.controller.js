...
export const register = asyncHandler(async (req, res) => {
  ...
  const profile = await prisma.profile.create({
    data: {
      name, email, passwordHash, authProvider: 'EMAIL',
...
export const emailLogin = asyncHandler(async (req, res) => {
  ...
  if (!profile) return res.status(401).json({ message: 'Credenciales inválidas.' });
...
export const unifiedLogin = asyncHandler(async (req, res) => {
  ...
  const token = signUserToken(profile.id);
  res.json({ token, role: 'user', profile: basicProfile(profile) });
});
