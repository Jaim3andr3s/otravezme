...
export const listPosts = asyncHandler(async (req, res) => {
  ...
  const [posts, total] = await Promise.all([
    prisma.forumPost.findMany({
      include: { likes: true, comments: true, _count: { select: { likes: true, comments: true } } },
  ...
  res.json(paginatedResponse(posts.map((p) => serializePost(p, req.auth?.profileId)), total, { page, limit }));
});
